...
이 문서의 기본 경로는 `/volume1/docker/Github/DHLottery`입니다. 이미 이 경로에 있다면 `/volume1/docker/DHLottery`로 이동하지 않아도 됩니다.
...
호스트에 Git이 설치되어 있다면 아래 명령으로 최신 코드를 받습니다.
...
호스트에 Git이 없으면 Docker로 `git pull --rebase`를 실행합니다.
...
일반 사용자로 실행했을 때 Docker socket 권한 오류가 나면 root shell에서 실행합니다.
