...
`python -m dhlottery_checker ... --notify`에서 `https://kauth.kakao.com/oauth/token` 401 오류가 나옵니다.
프로그램이 `KAKAO_REFRESH_TOKEN`으로 새 access token을 받는 단계에서 실패한 것입니다.
이미 채팅, 문서, 터미널 공유 화면 등에 노출된 refresh token은 사용하지 말고 새로 발급받으세요.
새 토큰을 받은 뒤 같은 PowerShell 창에서 아래 값을 다시 설정합니다.

```powershell
$env:KAKAO_REST_API_KEY = "REST_API_KEY"
$env:KAKAO_REFRESH_TOKEN = "새로_받은_REFRESH_TOKEN"

# Client Secret이 ON인 경우에만 설정하세요.
# $env:KAKAO_CLIENT_SECRET = "CLIENT_SECRET"
```
...
`No module named dhlottery_checker` 오류가 나옵니다.
PowerShell의 현재 위치가 저장소 폴더가 아닐 때 발생합니다.
아래 명령으로 저장소 폴더로 이동한 뒤 다시 실행하세요.
