...
class PagesHtmlTest(unittest.TestCase):
    ...
    def test_low_balance_status_pulses_background_smoothly(self):
        html = Path("docs/ticket-entry.html").read_text(encoding="utf-8")
        ...
        self.assertIn(".stat-value.balance-low::before", html)
        self.assertIn("background-color: var(--bad-alert-bg);", html)
        self.assertIn("background-color: var(--bad-alert-bg-strong);", html)
        self.assertIn("animation: balance-alert-background 1.6s ease-in-out infinite;", html)
        self.assertIn("will-change: opacity;", html)
        self.assertNotIn("steps(", html)
        self.assertNotIn("opacity: 0.25;", html)
        self.assertIn("@media (prefers-reduced-motion: reduce)", html)
        self.assertIn(".stat-value.balance-low::before {\n        animation: none;\n        opacity: 0.65;", html)
