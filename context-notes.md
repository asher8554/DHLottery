...
## 2026-06-13 최근 당첨결과 최신순 표시 보강

- 정확한 라이브 URL에서 `result-history.yml`과 DOM에는 319회가 들어와 있었다.
- 화면상 문제는 데이터 누락이 아니라 `최근 당첨결과` split 렌더링이 로또 컬럼을 항상 먼저 붙여 319회 연금복권 당첨 카드가 두 번째 카드로 밀리는 것이다.
- 이번 수정은 최신 이력 순서를 유지해 319회 연금복권 결과가 첫 카드로 보이게 하는 데 한정한다.
- 확인 중 원격 이력이 한 번 더 갱신되어 로또 1228회 미당첨 결과가 첫 카드가 됐다.
